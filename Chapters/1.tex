...
\begin{figure}
	\begin{center}
		...
		\caption[Preliminary $\theta_{23}$ and \text{sin}$^2\theta_{23}$ results from the NO$\nu$A Experiment]{ {\textbf{Preliminary $\theta_{23}$ and \text{sin}$^2\theta_{23}$ results from the NO$\nu$A Experiment}} \\ The 90\% confidence level region for $\theta_{23}$ and \text{sin}$^2\theta_{23}$, with the FC corrected allowed region and best-fit point for NO$\nu$A \cite{NOVA}, superposed on contours from other experiments.}
		\label{nova_result}	
	\end{center}
\end{figure} 
